# ── ACL policies ─────────────────────────────────────────────────────────────
# One resource per policy file — vault/policies/*.hcl and nomad/policies/*.hcl
# are the source of truth. Keyed on filename; add or remove a file to add or
# remove the policy, and the policy name is the filename minus the extension.
#
# tofu only manages — and only deletes — the policies backed by a file here.
# Built-ins (Vault `default`/`root`, Nomad `anonymous`) aren't file-backed, so
# tofu never touches them. A policy created out of band is likewise left alone.
#
