...
Available tags match role names: `trust`, `base`, `systemd`, `keepalived`, `consul`, `nomad`, `nomad_policies`, `vault`, `vault_policies`, `vault_agent`, `synology`. The `trust` role (home Root CA + Vault SSH client CA) is OS-agnostic and runs on every host; `base` (apt packages, editor, clusterdata NFS mount) is gated to Debian hosts so non-Debian nodes can take `trust` without it. The `vault_policies` role reconciles Vault ACL policies from `vault/policies/*.hcl` (source of truth: it writes each file and prunes any policy in Vault not backed by a file, except the built-ins in `vault_policies_protected`). It runs once from the control node against the Vault API using your operator token (`vault login -method=oidc`), so it needs `VAULT_TOKEN` in the environment — not the per-host SSH path the other roles use. The `nomad_policies` role does the same for Nomad ACL policies from `nomad/policies/*.hcl` (write-from-repo + prune, honoring `nomad_policies_protected`), running once from the control node against the Nomad API. Nomad ACL administration can't be granted by any policy, so it needs a **management** token in `NOMAD_TOKEN` (the break-glass subshell from `bin/nomad-mgmt`), not the day-to-day OIDC login.

Both policy-reconciliation roles carry the special `never` tag, so a full `site.yml` run (and `--tags all`) skips them — reconciliation can prune, so it must be opt-in. Invoke them explicitly with `--tags vault_policies` or `--tags nomad_policies` (after exporting the right token).
