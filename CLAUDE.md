...
| `bin/consul-mgmt`               | Break-glass into a lease-bound Consul **management** token subshell via the Vault Consul secrets engine (`consul/creds/mgmt`); revokes the lease on exit. Needed for Consul ACL administration and to seed `CONSUL_HTTP_TOKEN` for the `tofu` Consul modules |
